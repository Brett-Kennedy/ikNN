...
class ikNNClassifier(BaseEstimator, ClassifierMixin):
    """
    An sklearn-compatible predictor that aggregates predictions from a set of 2d spaces, using a standard
    ...
    def __init__(self, n_neighbors=8, method='use proba', weight_by_score=True, max_spaces_tested=50, num_best_spaces=5):
        ...
        max_spaces_tested: int
            ...
            as 1d kNN models. If set to -1, all spaces will be considered, which may cause the model to execute
            slowly.

        num_best_spaces: int
            The number of 2d kNN's used for predictions. If -1, a kNN will be created for every pair of features.
        """
        ...
        if self.max_spaces_tested == -1:
            self.max_spaces_tested = np.inf
        ...
        self.predict_arr = None
        self.subset_pred_arr_arr = None
        self.subset_pred_proba_arr_arr = None
    ...
    def fit(self, X, y):
        ...
        if X.ndim != 2 or X.shape[0] == 0 or X.shape[1] == 0:
            raise ValueError("Found array with 0 feature(s) (shape=(" + str(X.shape) + ") while a minimum "
                                                                                       "of 1 is required.")
        ...
        # subset of the pairs of columns. Determine the best 1d kNNs and test max_spaces_tested pairs of the top
        # 1d spaces.
        ...
        if (self.num_columns * (self.num_columns-1) / 2) > self.max_spaces_tested:
            ...
            num_cols_used = ceil(sqrt(self.max_spaces_tested * 2))
        ...
        self.classes_ = self.knn_arr[0].classes_
    ...
    def predict(self, X):
        ...
        # predict_arr: contains the final prediction per row
        # subset_pred_arr_arr: contains an array with the final prediction of each 2d kNN for each row.
        # subset_pred_proba_arr_arr: contains an element for each 2d kNN for each class for each row.
        self.predict_arr = []
        ...
        for row_idx in range(len(X)):
            ...
            #proba_collection = proba_collection * proba_collection
        ...
        # The simple majority vote among the 2d kNN's.
            ...
            # Calculate the average probability per class among the 2d kNNs and take the class with the
    ...
    def __graph_2d_spaces(self, row=None, row_idx=None, true_class=None, feature_names=None, num_spaces_shown=-1):
        """
          Parameters
            ----------
            row: pandas series
            ...
            feature_names: list of str
                The names to be displayed in the scatterplot titles
        ...
        fig, ax = plt.subplots(nrows=1, ncols=num_spaces_shown, figsize=(num_spaces_shown*5, 5))
        y_values = self.knn_arr[0].classes_
        x_df = pd.DataFrame(self.X)
        y_series = pd.Series(self.y)
        if row_idx is not None:
            if (self.subset_pred_arr_arr is None) or (self.subset_pred_arr_arr == []):
                print("Must call predict() for explaining predictions")
                return
        ...
        for plot_idx in range(num_spaces_shown):
            ...
            mesh_pred = clf.predict(df.values)
            for i, uv in enumerate(y_values):
                mesh_pred[mesh_pred == uv] = i
            ax[plot_idx].contourf(x_mesh, y_mesh, mesh_pred.reshape(x_mesh.shape), colors=tableau_palette_list, alpha=0.1)

            ax[plot_idx].legend()
            if row_idx:
                ax[plot_idx].set_title("Columns: \n" + str(feature_names[col_pair[0]]) + ", \n" + str(feature_names[col_pair[1]]) + " -- Predicted: " + str(pred_by_space[plot_idx]))
            else:
                ax[plot_idx].set_title("Columns: \n" + str(feature_names[col_pair[0]]) + ", \n" + str(feature_names[col_pair[1]]))
        ...
        if row_idx is not None:
            ...
            fig_title = "Model Overview"
        fig.suptitle(fig_title)
        plt.tight_layout()
        plt.show()

    def graph_model(self, feature_names, num_spaces_shown=-1):
        """
        Provides a global overview of the model, not considering any specific predictions.
        """
        self.__graph_2d_spaces(feature_names=feature_names, num_spaces_shown=num_spaces_shown)

    def graph_predictions(self, row, row_idx, true_class, feature_names, num_spaces_shown=-1):
        """
        ...
        self.__graph_2d_spaces(row, row_idx, true_class, feature_names, num_spaces_shown)